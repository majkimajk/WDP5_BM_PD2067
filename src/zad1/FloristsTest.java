...
public class FloristsTest {
  ...
  static int valueOf(Box box, String color) {
	  double cenaKoloru = 0;
	  double cenaKwiatu  = 0;
	  for (String nazwa : box.zawartosc.keySet()) {
		for (String nazwa2 : box.kolory.keySet()) {
			if (nazwa == nazwa2){
				
				if (box.kolory.get(nazwa2) == color) {
					
					cenaKwiatu = PriceList.getInstance().mapa.get(nazwa2);				
					cenaKoloru = cenaKoloru + cenaKwiatu * box.zawartosc.get(nazwa);
				}
			}
		}
